...
class DiscoveredFieldBinding extends DiscoveredBinding {
  ...
  factory DiscoveredFieldBinding.parse(FieldDeclaration fieldDeclaration) {
    ...
    return new DiscoveredFieldBinding(implementedType, concreteType, true, initializer);
  }

  DiscoveredFieldBinding(this.implementedType, this.concreteType, this.isSingleton, this.initializer);
...
class DiscoveredMethodBinding extends DiscoveredBinding {
  ...
  factory DiscoveredMethodBinding.parse(MethodDeclaration methodDeclaration) {
    ...
    if (methodDeclaration.body is ExpressionFunctionBody) {
      if ((methodDeclaration.body as ExpressionFunctionBody).expression is PropertyAccess) {
        singleton = ((methodDeclaration.body as ExpressionFunctionBody).expression as PropertyAccess).propertyName.toString() == 'singleton';
      }
    }
    if (methodDeclaration.body is EmptyFunctionBody) {
      ...
      if (expressionBody.expression is PropertyAccess) {
        ...
        if(target is SimpleIdentifier) {
          ...
          concreteType = ((target as SimpleIdentifier).staticElement as ClassElement).type;
        ...
        if ((target.target as MethodInvocation).argumentList.arguments[0] is SimpleIdentifier) {
          ...
          concreteType = (((target.target as MethodInvocation).argumentList.arguments[0] as SimpleIdentifier).staticElement as ClassElement).type;
          ...
          initializer = ((target.target as MethodInvocation).argumentList.arguments[0] as FunctionExpression);
          concreteType = ((target.target as MethodInvocation).argumentList.arguments[0] as FunctionExpression).element.returnType;
        }
      }
    }
    //enumerate dependency graph in the args so we can check all types our bound later
    constructorArgs.addAll((concreteType.element as ClassElement).constructors.first.parameters);
    transitiveDependencies = _enumerateDepenedenyGraph(constructorArgs);
    return new DiscoveredMethodBinding(implementedType, concreteType, singleton, initializer, new UnmodifiableListView(constructorArgs), new UnmodifiableSetView(transitiveDependencies));
  }

  //TODO(bendera): this could improved using some memoization
  static Set<Type2> _enumerateDepenedenyGraph(List<ParameterElement> graphRoots) =>
    graphRoots.expand((ParameterElement el) =>
      _enumerateDepenedenyGraph((el.type.element as ClassElement).constructors[0].parameters)..add(el.type)).toSet();


  DiscoveredMethodBinding(this.implementedType, this.concreteType, this.isSingleton, this.initializer, this.constructorArgs, this.transitiveDependencies);
  ...
  String toString() => "DiscoveredMethodBinding[ImplementedType: $implementedType, ConcreteType: $concreteType, Initializer: $initializer, singleton: $isSingleton, constructorArgs: ${constructorArgs.toList()}, transitiveDependencies: ${transitiveDependencies.toSet()}]";
}